...
mysql = mysql.connector.connect(host="localhost", user="root", password="Bisoyi@6092", database="feemanagement")
...
def registration():
    if request.method == 'POST':
        ...
        try:
            ...
            confirm_url = url_for('confirm', token=token(data, salt), _external=True)
            subject = 'Email Confirmation'
            body = f"Thanks for signing up!\n\nPlease confirm your registration by clicking the link below:\n{confirm_url}"

            sendmail(to=email, subject=subject, body=body)

            # Optional: log the link for local dev
            print(f"[DEBUG] Confirmation link: {confirm_url}")

            flash('Confirmation link sent to your email. Please verify to complete registration.')
            return redirect(url_for('login'))
